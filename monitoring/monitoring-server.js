...
// GET: List active rawat jalan visits today
app.get('/api/patients/outpatient/active', requireAdminOrPerawat, async (req, res) => {
  let conn;
  try {
    conn = await pool.getConnection();
    ...
    let query = `
      SELECT 
        ...
        latest_v.glukosa,
        latest_v.waktu as waktu_vital,
        (SELECT COUNT(*) FROM vitals v WHERE v.emr_no = k.emr_no 
         AND DATE(v.waktu) = DATE(k.tanggal_kunjungan)) as total_measurements
      FROM kunjungan k
      ...
      ) latest_v ON k.emr_no = latest_v.emr_no
      WHERE k.tanggal_kunjungan >= ? 
        AND k.tanggal_kunjungan < ?
        AND k.status = 'aktif'
    `;
...
// GET: Detail rawat jalan patient by id_kunjungan
app.get('/api/patients/outpatient/:id_kunjungan', requireAdminOrPerawat, async (req, res) => {
  let conn;
  try {
    ...
    const [latestVital] = await conn.query(`
      SELECT 
        heart_rate, respirasi, sistolik, diastolik, 
        glukosa, berat_badan_kg, tinggi_badan_cm, bmi,
        fall_detected, waktu
      FROM vitals
      WHERE emr_no = ?
        AND DATE(waktu) = DATE(?)
...
// GET: Vitals history for this visit (chart data)
app.get('/api/patients/outpatient/:id_kunjungan/vitals/chart', requireAdminOrPerawat, async (req, res) => {
  let conn;
  try {
    const { id_kunjungan } = req.params;
    ...
    // Get visit info
    const [visit] = await conn.query(`
      SELECT emr_no, tanggal_kunjungan 
      FROM kunjungan 
      WHERE id_kunjungan = ?
    `, [id_kunjungan]);
    ...
    const { emr_no, tanggal_kunjungan } = visit[0];
    
    // Get all vitals for this visit (same day)
    const [vitals] = await conn.query(`
      SELECT 
        waktu,
        heart_rate,
...
// GET: All examinations for this visit
app.get('/api/patients/outpatient/:id_kunjungan/examinations', requireAdminOrPerawat, async (req, res) => {
  let conn;
  try {
    const { id_kunjungan } = req.params;
